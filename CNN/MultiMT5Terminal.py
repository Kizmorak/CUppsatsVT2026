...
def run_trader(path, login, password, server, TIConfiguration, modelName, reverseTransaction):
    ...
    modelFullPath = TIConfiguration+modelName
    model = test_model.TestingModel(modelFullPath)
    lastCandle = waitForNewCandle(ratesSymbol, ratesTimeFrame)
    target_columns = [
        'open', 'high', 'low', 'close', 'tick_volume', 'spread', 'real_volume',
        'ma15', 'bbmiddle', 'stddev', 'bbupper', 'bblower', 'delta', 'up',
        'down', 'upmean', 'downmean', 'rs', 'rsi', 'direction', 'obv',
        'prevclose', 'tr1', 'tr2', 'tr3', 'truerange', 'averagetruerange'
    ]
    try:
        while True:
            #current_min = datetime.datetime.now().minute
            currencCandle = waitForNewCandle(ratesSymbol, ratesTimeFrame)
            if currencCandle is not None and currencCandle > lastCandle and datetime.time(9, 0) <= datetime.datetime.now().time() <= datetime.time(15):
                ...
                saveFolderName = os.path.join("inputGraph", TIConfiguration+modelName)
                ...
                if order_type == mt5.ORDER_TYPE_BUY or order_type == mt5.ORDER_TYPE_SELL:
                    ...
                    if result.retcode != mt5.TRADE_RETCODE_DONE:
                        ...
                        market_context = {}
                        ratesData.columns = [col.lower() for col in ratesData.columns]

                        # 2. Define the columns you want to save from the list you provided
                        # (Using lowercase here since we renamed them above)


                        # 3. Loop through rows and dynamically add every indicator to the context
                        for i, row in ratesData.iterrows():
                            for col in target_columns:
                                if col in row:
                                    market_context[f'tick_{i}_{col}'] = row[col]

                        # 4. Merge dictionaries (Context + Request + Result)
                        csvData = {**market_context, **request, **result._asdict()}
                        csvdf = pd.DataFrame([csvData])

                        csv_file = TIConfiguration+modelName+'trade_dataset.csv'
            ...
            # --- POSITION EXPIRY CHECK WITH LOGGING ---
            positions = mt5.positions_get(symbol=ratesSymbol)

            if positions:
                for pos in positions:
                    current_time = int(time.time())
                    duration_seconds = current_time - pos.time

                    if duration_seconds >= 1200:  # 20 minutes
                        # 1. Prepare Close Request
                        ...
                        # 3. Log to CSV if successful
                        if result.retcode == mt5.TRADE_RETCODE_DONE:
                            print(f"Closed position {pos.ticket} due to time limit.")

                            # Capture current market state (indicators) for the close log
                            close_context = {}
                            # Grab the last row of indicators from your ratesData
                            last_row = ratesData.iloc[-1]
                            for col in target_columns:  # Uses the list defined in your original script
                                if col in last_row:
                                    close_context[f'close_tick_{col}'] = last_row[col]

                            # Merge dictionaries
                            csv_close_data = {**close_context, **close_request, **result._asdict()}
                            csv_close_df = pd.DataFrame([csv_close_data])

                            # Append to the same CSV file
                            csv_file = TIConfiguration + modelName + 'trade_dataset.csv'
                            csv_close_df.to_csv(csv_file, mode='a', index=False, header=not os.path.isfile(csv_file))
    ...
    account_configs = [
        {
            ...
            "login": 10010690149, "password": "-8RrJbTu", "server": "MetaQuotes-Demo",
            "TIConfiguration": "No_TI_104_26_10_10_20251224_10_1_4",
            "modelName": "__20260422_221121",
            "reverseTransaction": 0
        },
        {
            "path": r"N:\MT5Terminals\Terminal2\MetaTrader 5\terminal64.exe",
            "login": 10010690155, "password": "@xJdU8Tv", "server": "MetaQuotes-Demo",
            "TIConfiguration": "No_TI_104_26_10_10_20251224_10_1_4",
            "modelName": "__20260422_221637",
            "reverseTransaction": 1
        },
        {
            "path": r"N:\MT5Terminals\Terminal3\MetaTrader 5\terminal64.exe",
            "login": 10010690161, "password": "*2KpUfZe", "server": "MetaQuotes-Demo",
            "TIConfiguration": "BB_104_26_10_10_20251224_10_1_4",
            "modelName": "__20260423_213608",
            "reverseTransaction": 1
        },
        {
            "path": r"N:\MT5Terminals\Terminal4\MetaTrader 5\terminal64.exe",
            "login": 10010690165, "password": "AtXi@3Zv", "server": "MetaQuotes-Demo",
            "TIConfiguration": "BB_104_26_10_10_20251224_10_1_4",
            "modelName": "__20260423_214509",
            "reverseTransaction": 1
        },
        {
            "path": r"N:\MT5Terminals\Terminal5\MetaTrader 5\terminal64.exe",
            "login": 10010690166, "password": "FdJy*8Et", "server": "MetaQuotes-Demo",
            "TIConfiguration": "OBV_104_26_10_10_20251224_10_1_4",
            "modelName": "__20260424_095029",
            "reverseTransaction": 0
        },
        {
            "path": r"N:\MT5Terminals\Terminal6\MetaTrader 5\terminal64.exe",
            "login": 10010690172, "password": "JqW*Hb4t", "server": "MetaQuotes-Demo",
            "TIConfiguration": "OBV_104_26_10_10_20251224_10_1_4",
            "modelName": "__20260424_095811",
            "reverseTransaction": 1
        },
        {
            "path": r"N:\MT5Terminals\Terminal7\MetaTrader 5\terminal64.exe",
            "login": 10010690175, "password": "7uHnUu-g", "server": "MetaQuotes-Demo",
            "TIConfiguration": "RSI_104_26_10_10_20251224_10_1_4",
            "modelName": "__20260423_211805",
            "reverseTransaction": 1
        },
        {
            "path": r"N:\MT5Terminals\Terminal8\MetaTrader 5\terminal64.exe",
            "login": 10010690180, "password": "Vi-0CwTw", "server": "MetaQuotes-Demo",
            "TIConfiguration": "RSI_104_26_10_10_20251224_10_1_4",
            "modelName": "__20260423_212722",
            "reverseTransaction": 1
        }
