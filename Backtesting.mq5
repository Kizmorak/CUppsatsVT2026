...
int OnInit()
  {
  string fileName = "backtesting_predictions_2.csv";
   ...
   if(fileHandle != INVALID_HANDLE) {
      ...
      for(int i = 0; i < totalSignals; i++)
      {
         ...
         if(signals[i].type == 0) signalType = "BUY";
         else if(signals[i].type == 1) signalType = "SELL";
...
void OnTick()
{
   ...
   for(int i = 0; i < ArraySize(signals); i++) {
      ...
      if(signals[i].time == currentTime && strTime.hour >= 9 && strTime.hour <= 15 ) {
